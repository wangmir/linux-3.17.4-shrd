...
int scsi_shrd_map_insert(struct SHRD_MAP_HEAD *mapping, struct SHRD_MAP *map_entry){
	...
	while (*new){
		...
		int result = (int)map_entry->o_addr - (int)this->o_addr;
...
//temporal test for fewer twrite entries
	for(idx = 0; idx < (SHRD_TWRITE_ENTRIES); idx++){
		...
		sdev->shrd->twrite_cmd[idx].twrite_hdr = (struct SHRD_TWRITE_HEADER *) __get_free_page(GFP_KERNEL);
	...
	for(idx = 0; idx < SHRD_REMAP_ENTRIES; idx++){
		...
		for(iter = 0; iter < SHRD_REMAP_DATA_PAGE; iter++){
			sdev->shrd->remap_cmd[idx].remap_data[iter] = (struct SHRD_REMAP_DATA *) __get_free_page(GFP_KERNEL);
...
static struct SHRD_TWRITE * scsi_shrd_prep_rw_twrite(struct request_queue *q, struct request *rq){
	...
	if(shrd->rw_log_new_idx >= shrd->rw_log_start_idx){
		...
		if((SHRD_RW_LOG_SIZE_IN_PAGE - shrd->rw_log_new_idx) >= SHRD_MIN_RW_LOGGING_IO_SIZE_IN_PAGE){
			...
			if(shrd->rw_log_start_idx >= SHRD_MAX_TWRITE_IO_SIZE_IN_PAGE){
...
static void scsi_shrd_bio_endio(struct bio* bio, int err){
	...
	if(bio->bi_rw & REQ_SHRD_TWRITE_HDR){
		...
		shrd_dbg_printk(KERN_INFO, sdev, "%d: %s: twrite data completion: twrite: %llx, twrite block: %d, nr_request: %d, phys_segment: %d\n", smp_processor_id(), __func__,(u64)twrite_entry,  twrite_entry->blocks, twrite_entry->nr_requests, twrite_entry->phys_segments);
		...
		shrd_dbg_printk(KERN_INFO, sdev, "%d: %s: remap data dump: entry %llx, count %u\n", smp_processor_id(), __func__, (u64)remap_entry,
...
static struct bio* scsi_shrd_make_twrite_data_bio(struct request_queue *q, struct SHRD_TWRITE *twrite_entry){
	...
	bio->bi_max_vecs = SHRD_NUM_MAX_TWRITE_ENTRY;
...
static void  scsi_shrd_make_remap_bio(struct request_queue *q, struct SHRD_REMAP *remap_entry){
	...
	for(i=0; i < SHRD_NUM_CORES; i++){
		if(bio_add_page(bio, virt_to_page((void *)remap_entry->remap_data[0]), PAGE_SIZE, 0) < PAGE_SIZE){
			sdev_printk(KERN_INFO, sdev, "%s: SHRD bio_add_pc_page failed on CORE %d\n", __func__, i);
			BUG();
		}
	}

	shrd_dbg_printk(KERN_INFO, sdev, "%d: %s: dump remap data, count: %u\n", smp_processor_id(), __func__,
...
static void __scsi_shrd_do_remap_rw_log(struct request_queue *q, u32 size){
	...
	shrd_dbg_printk(KERN_INFO, sdev, "%d: %s: do_remap, start_idx: %u, size %u\n",smp_processor_id(),  __func__, start_idx, size);
	...
	for(node = rb_first(&shrd->rw_mapping.rbtree_root); node; node = rb_next(node)){
		...
		if((map->t_addr >= start_idx + SHRD_RW_LOG_START_IN_PAGE) && (map->t_addr <= end_idx + SHRD_RW_LOG_START_IN_PAGE)){
			...
			if(entry->remap_data[0]->remap_count == SHRD_NUM_MAX_REMAP_ENTRY){
				...
				entry = shrd_get_remap_entry(shrd);
				if(entry == NULL){
					sdev_printk(KERN_ERR, sdev, "%s: has no remap entry\n", __func__);
					BUG();
					return;
				}
				list_del(&entry->remap_cmd_list);
			...
			entry->remap_data[0]->remap_count++;
			idx++;
		}
...
static struct SHRD_REMAP* scsi_shrd_prep_remap_if_need(struct request_queue *q){
	...
	if(size > shrd->remap_threshold){
		...
		//BUG_ON(list_empty(&shrd->ongoing_remap_cmd_list));
		return list_first_entry_or_null(&shrd->ongoing_remap_cmd_list, struct SHRD_REMAP, remap_cmd_list);
	}
	...
	for (;;) {
		...
		if(sdev->shrd_on){
			...
			if(bio && bio->bi_rw & REQ_SHRD_REMAP){
				...
				if((remap_entry = list_first_entry_or_null(&sdev->shrd->ongoing_remap_cmd_list, struct SHRD_REMAP, remap_cmd_list))){
					list_del(&remap_entry->remap_cmd_list);
					scsi_shrd_make_remap_bio(q, remap_entry);
					shrd_dbg_printk(KERN_INFO, sdev, "%d: %s: SHRD insert another remap entry\n", smp_processor_id(), __func__);
				}
				goto spcmd;
			}
